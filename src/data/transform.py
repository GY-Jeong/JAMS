...
class OneHotEncoder(Transform):
    ...
    def save(self, path: str, version: str) -> None:
        ...
        """
        self.file_name = self.file_name.split('.')[0] + f"_{version}." + self.file_name.split('.')[1]
        path = Path(path) / self.file_name
        with open(path, "w") as f:
            json.dump(self.target2index, f)

    def load(self, path: str, version: str) -> None:
    ...
    def generate_invalid_tensor(self) -> torch.Tensor:
        """Generate a tensor filled with -1, with length of self.num_classes.

        Returns:
            torch.Tensor: A 1D tensor of shape (self.num_classes,) filled with -1.
        """
        output_tensor = torch.full((self.num_classes,), -1)
        return output_tensor
