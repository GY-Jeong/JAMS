...
class Trainer:
    ...
    def fit(self) -> None:
        ...
        try:
            ...
            for _ in range(self.epoch, self.epochs):
                ...
                self.val(self.epoch, "val", version=version)
                self.on_epoch_end(version)
                self.epoch += 1
            self.on_fit_end()
            self.val(self.epoch, "val", version='mimiciv_icd9', evaluating_best_model=True)
            self.val(self.epoch, "test", version='mimiciv_icd9', evaluating_best_model=True)
            self.val(self.epoch, "val", version='mimiciv_icd10', evaluating_best_model=True)
            self.val(self.epoch, "test", version='mimiciv_icd10', evaluating_best_model=True)
    ...
    def train_one_epoch(self, epoch: int, version: str) -> None:
        ...
        self.model.train()
        self.on_train_begin()
        num_batches = len(self.dataloaders[version]["train"])
        for batch_idx, batch in enumerate(
            track(self.dataloaders[version]["train"], description=f"Epoch: {epoch} | {version} Training")
        ):
            ...
            if ((batch_idx + 1) % self.accumulate_grad_batches == 0) or (
                ...
                self.gradient_scaler.step(self.optimizer[version])
                self.gradient_scaler.update()
                if self.lr_scheduler[version] is not None:
                    if not isinstance(
                        self.lr_scheduler[version], torch.optim.lr_scheduler.ReduceLROnPlateau
                    ):
                        self.lr_scheduler[version].step()
                self.optimizer[version].zero_grad()
            self.update_metrics(output, "train", version)
    ...
    def update_metrics(self, outputs: dict[str, torch.Tensor], split_name: str, version: str) -> None:
        for target_name in self.metric_collections[version][split_name].keys():
            self.metric_collections[version][split_name][target_name].update(outputs)
            ...
            if version == 'mimiciv_icd9':
                ...
                if evaluating_best_model:
                    ...
                    for target_name in self.metric_collections[version]["test"]:
                        self.metric_collections[version]["test"][target_name].set_threshold(best_db)
    ...
    def save_predictions(
        ...
        label_transform = self.dataloaders[version][split_name].dataset.label_transform
