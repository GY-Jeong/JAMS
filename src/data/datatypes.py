...
class Data:
    ...
    def from_batches_to_list(
        ...
        for batch in track(batches, description="Creating examples"):
            ...
            examples.append((token_ids, targets, id, num_tokens, attention_mask, version))
        return examples
    ...
    def all_targets(self) -> set[str]:
        ...
        # 코드의 순서를 유지하기 위해 리스트 형태로 저장하도록 수정
...
class Batch:
    ...
    data: torch.Tensor
    targets_icd9: torch.Tensor
    targets_icd10: torch.Tensor
    ids: torch.Tensor
    ...
    def to(self, device: Any) -> "Batch":
        ...
        self.targets_icd9 = self.targets_icd9.to(device, non_blocking=True)
        self.targets_icd10 = self.targets_icd10.to(device, non_blocking=True)
    ...
    def pin_memory(self):
        ...
        self.targets_icd9 = self.targets_icd9.pin_memory()
        self.targets_icd10 = self.targets_icd10.pin_memory()
        if self.attention_mask is not None:
