...
class Trainer:
    def __init__(
        ...
        self.device = "cpu"
        self.all_metric_collections = all_metric_collections
        self.metric_collections = None
        ...
        try:
            ...
            for _ in range(self.epoch, self.epochs):
                if self.stop_training:
                    break
                # TODO : epoch에 따라서 학습 데이터셋 변경
                version = 'mimiciv_icd9' if self.epoch % 2 == 0 else 'mimiciv_icd10'

                self.on_epoch_begin(version)
                self.train_one_epoch(self.epoch, version=version)
                if self.validate_on_training_data:
                    self.train_val(self.epoch, "train_val", version)
                self.val(self.epoch, version=version, split_name="val")
                self.on_epoch_end(version)
                self.epoch += 1
            self.on_fit_end()
    ...
    def train_one_epoch(self, epoch: int, version: str) -> None:
        ...
        for batch_idx, batch in enumerate(
            track(self.dataloaders["train"], description=f"Epoch: {epoch} | {version} Training")
        ):
            batch = batch.to(self.device)
            with torch.autocast(
                device_type="cuda", dtype=torch.float16, enabled=self.use_amp
            ):
                output = self.model.training_step(batch, version)
        ...
        self.on_train_end(epoch, version)

    def train_val(self, epoch, version: str, split_name: str = "train_val") -> None:
        ...
        with torch.no_grad():
            for batch in track(
                ...
                description=f"Epoch: {epoch} | Validating on training data & {version}",
            ):
                with torch.autocast(
                    device_type="cuda", dtype=torch.float16, enabled=self.use_amp
                ):
            ...
            self.on_val_end(split_name, epoch, version)
    ...
    def val(
        ...
        self.metric_collections = self.all_metric_collections[version]
        self.optimizer = self.all_optimizers[version]
        self.lr_scheduler = self.all_lr_schedulers[version]
        self.lookups = self.all_lookups[version]
        self.dataloaders = self.all_dataloaders[version]
            ...
            if version == 'mimiciv_icd9':
                ...
                if evaluating_best_model:
                    ...
                    for target_name in self.metric_collections["test"]:
                        self.metric_collections["test"][target_name].set_threshold(best_db_icd9)
                ...
                if evaluating_best_model:
                    ...
                    for target_name in self.metric_collections["test"]:
                        self.metric_collections["test"][target_name].set_threshold(best_db_icd10)
    ...
    def reset_metrics(self, version) -> None:
    ...
    def on_train_end(self, epoch: int, version: str) -> None:
        results_dict = self.calculate_metrics(split_name="train", version=version)
    ...
    def on_epoch_begin(self, version) -> None:
        self.reset_metrics(version)
        for callback in self.callbacks:
            callback.on_epoch_begin(self)

    def on_epoch_end(self, version) -> None:
        if self.lr_scheduler is not None:
            if isinstance(
                self.lr_scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau
            ):
        ...
        for callback in self.callbacks:
            callback.on_epoch_end(self, version=version)
    ...
    def to(self, device: str) -> "Trainer":
        ...
        for split_name in self.all_metric_collections['mimiciv_icd10'].keys():
            for target_name in self.all_metric_collections['mimiciv_icd10'][split_name].keys():
                self.all_metric_collections['mimiciv_icd10'][split_name][target_name].to(device)

        for split_name in self.all_metric_collections['mimiciv_icd9'].keys():
            for target_name in self.all_metric_collections['mimiciv_icd9'][split_name].keys():
                self.all_metric_collections['mimiciv_icd9'][split_name][target_name].to(device)
        self.device = device
        return self
    ...
    def load_checkpoint(self, file_name: str, version: str) -> None:
        ...
        checkpoint = torch.load(f'/home/gy/workspace/AMC/medical-coding-reproducibility/experiments/0ukxmicw/best_model_{version}.pt')
        # print(checkpoint)/
    ...
    def save_transforms(self, version) -> None:
        """Save text tokenizer and label encoder"""
        self.dataloaders[version]["train"].dataset.text_transform.save(self.experiment_path)
        self.dataloaders[version]["train"].dataset.label_transform.save(self.experiment_path)
