...
def data_pipeline(config: OmegaConf, version: str) -> Data:
    """The data pipeline.

    Args:
        config (OmegaConf): The config.
        version (str): The version string to be added to the df.
    ...
    with vaex.cache.memory_infinite():  # type: ignore
        ...
        # version: 'mimiciv_icd9' or 'mimiciv_icd10'
        version = version.split('_')[1]

        df['version'] = np.array([version] * len(df))
