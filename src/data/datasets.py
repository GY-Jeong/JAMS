...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, int, int]:
        token_ids, targets, id, num_tokens, _, version = self.data[idx]
        if version == 'icd9':
            targets_icd9 = self.label_transform['mimiciv_icd9'](targets)
            targets_icd10 = self.label_transform['mimiciv_icd10'].generate_invalid_tensor()
        else:
            targets_icd9 = self.label_transform['mimiciv_icd9'].generate_invalid_tensor()
            targets_icd10 = self.label_transform['mimiciv_icd10'](targets)
            
        return token_ids, targets_icd9, targets_icd10, id, num_tokens, version

    def collate_fn(self, batch: tuple[list, list, list, list, list, list]) -> Batch:
        data, targets_icd9, targets_icd10, ids, num_tokens, version = zip(*batch)
        data = self.text_transform.seq2batch(data)
        targets_icd9 = self.label_transform['mimiciv_icd9'].seq2batch(targets_icd9)
        targets_icd10 = self.label_transform['mimiciv_icd10'].seq2batch(targets_icd10)
        ids = torch.tensor(ids)
        num_tokens = torch.tensor(num_tokens)
        return Batch(data=data, targets_icd9=targets_icd9, targets_icd10=targets_icd10, ids=ids, num_tokens=num_tokens, version=version)
...
class HuggingfaceDataset(Dataset):
    ...
    def __getitem__(
        ...
        token_ids, targets, id, num_tokens, attention_mask, version = self.data[idx]
        if version == 'icd9':
            targets_icd9 = self.label_transform['mimiciv_icd9'](targets)
            targets_icd10 = self.label_transform['mimiciv_icd10'].generate_invalid_tensor()
        else:
        ...
        return token_ids, targets_icd9, targets_icd10, id, num_tokens, attention_mask, version

    def collate_fn(self, batch: tuple[list, list, list, list]) -> Batch:
        token_ids, targets_icd9, targets_icd10, ids, num_tokens, attention_mask, version = zip(*batch)
        ...
        return Batch(data=data, targets_icd9=targets_icd9, targets_icd10=targets_icd10, ids=ids, num_tokens=num_tokens, version=version, attention_mask=attention_mask)
